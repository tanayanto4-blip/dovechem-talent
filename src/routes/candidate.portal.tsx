  ...
  head: () => ({
    meta: [
      { title: "Portal Kandidat — PT Dover Chemical" },
      {
        name: "description",
        content:
          "Portal kandidat PT Dover Chemical: lengkapi data diri, dengarkan instruksi, lalu kerjakan rangkaian psikotest online sesuai jadwal HR.",
      },
...
function PortalLayout() {
  ...
  useEffect(() => {
    ...
    reportIncident(
      ...
      "Kandidat keluar otomatis: kode dipakai di perangkat lain",
      {
        code: session?.code,
  ...
  useEffect(() => {
    ...
    const goOffline = () => {
      ...
      reportIncident("koneksi-terputus", "Koneksi internet kandidat terputus", {
        code: session?.code,
      });
    };
    const goOnline = () => {
      ...
      reportIncident("koneksi-pulih", "Koneksi internet kandidat kembali normal", {
        code: session?.code,
      });
    };
  ...
  useEffect(() => {
    ...
    reportIncident(
      "sesi-tidak-valid",
      `Sesi kandidat ditolak: ${(sessionError as Error).message}`,
      {
        code: session?.code,
      },
    );
    ...
    <div className="touch-ui min-h-screen overflow-x-hidden bg-subtle">
          ...
          <Link to="/" className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              ...
              <div className="truncate text-[10px] uppercase tracking-widest text-muted-foreground">
                {candidateTypeLabel(session?.type, (profile as any)?.job_level)}
              </div>
            </div>
          </Link>
      ...
      {offline && (
        <div className="bg-destructive px-4 py-2 text-center text-xs font-medium text-destructive-foreground sm:px-6 sm:text-sm">
          Koneksi internet terputus. Jangan tutup halaman — jawaban tersimpan otomatis saat sinyal
          kembali.
        </div>
      )}
          ...
          {nav_items.map((it) => {
            ...
            return (
              <Link
                key={it.to}
                to={it.to}
                className={`flex items-center gap-3 rounded-md px-3 py-2 text-sm font-medium transition-colors ${active ? "bg-primary text-primary-foreground" : "text-muted-foreground hover:bg-accent hover:text-accent-foreground"}`}
              >
          ...
          {sessionError ? (
            <div className="rounded-lg border border-destructive/40 bg-destructive/10 p-6 text-center">
              <div className="font-semibold text-destructive">
                Sesi kandidat tidak dapat digunakan
              </div>
