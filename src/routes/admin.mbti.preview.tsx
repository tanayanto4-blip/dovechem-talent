  ...
  head: () => ({
    meta: [
      { title: "Preview Set Soal MBTI — Admin" },
      {
        name: "description",
        content:
          "Ringkasan set soal MBTI yang sedang berstatus publish beserta preview tampilan kandidat.",
      },
  ...
  const stats = useMemo(() => {
    ...
    questions.forEach((q) => {
      ...
      const validPair =
        ...
        DIM_PAIRS.some(
          (p) =>
            (p.labels as readonly string[]).includes(ad) &&
            (p.labels as readonly string[]).includes(bd),
        );
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Ringkasan set soal yang sedang berstatus publish beserta tampilan yang akan dilihat
            kandidat.
          </p>
        </div>
        {mbtiTests.length > 1 && (
      ...
      {loadingTests ? (
        ...
        <div className="rounded-md border bg-muted/40 p-6 text-sm text-muted-foreground">
          Pilih test untuk melihat preview.
        </div>
      ) : (
        <>
            ...
            <CardContent className="flex flex-wrap items-center justify-between gap-4 p-4">
              ...
              {!activeTest.active && (
                <div className="flex items-center gap-2 rounded-md border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning-foreground">
                  <AlertTriangle className="h-4 w-4" /> Test ini belum dipublikasikan. Aktifkan
                  toggle Publish di Bank Soal untuk merilis.
                </div>
              )}
            </CardContent>
          </Card>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <Card className="shadow-card">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Distribusi Dimensi</CardTitle>
              </CardHeader>
              <CardContent>
                      ...
                      <div key={p.key}>
                        <div className="mb-1 flex items-center justify-between text-xs">
                          ...
                          <span className="font-mono">
                            {x}: <b>{cx}</b> · {y}: <b>{cy}</b>{" "}
                            {balanced && <span className="ml-1 text-success">seimbang</span>}
                          </span>
                        </div>
            ...
            <Card className="shadow-card">
              <CardHeader className="pb-2">
                <CardTitle className="text-base">Cek Kualitas Set</CardTitle>
              </CardHeader>
              <CardContent>
                <ul className="space-y-2 text-sm">
                  <QaRow
                    ok={stats.total > 0}
                    okText={`${stats.total} soal terdaftar`}
                    failText="Belum ada soal"
                  />
                  <QaRow
                    ok={stats.dups.length === 0}
                    okText="Tidak ada nomor ganda"
                    failText={`Nomor duplikat: ${stats.dups.join(", ")}`}
                  />
                  <QaRow
                    ok={stats.missing.length === 0}
                    okText="Nomor urut lengkap"
                    failText={`Nomor hilang: ${stats.missing.slice(0, 12).join(", ")}${stats.missing.length > 12 ? "…" : ""}`}
                  />
                  <QaRow
                    ok={stats.invalid === 0}
                    okText="Semua pasangan dimensi valid"
                    failText={`${stats.invalid} soal dengan pasangan dimensi tidak valid`}
                  />
                  <QaRow
                    ok={stats.empty === 0}
                    okText="Semua pernyataan terisi"
                    failText={`${stats.empty} soal dengan pernyataan kosong`}
                  />
                </ul>
              </CardContent>
            </Card>
          ...
          <Card className="shadow-card">
            <CardHeader className="pb-2 flex flex-row items-center justify-between">
              <CardTitle className="text-base flex items-center gap-2">
                <Eye className="h-4 w-4" /> Preview Tampilan Kandidat
              </CardTitle>
