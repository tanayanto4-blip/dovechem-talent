  ...
  head: () => ({
    meta: [
      { title: "Kode Akses Kandidat — Admin Dover Chemical" },
      {
        name: "description",
        content:
          "Buat, aktifkan, dan atur masa berlaku kode akses login kandidat psikotest PT Dover Chemical.",
      },
...
function CodesPage() {
  ...
  const [form, setForm] = useState({
    ...
    code: "",
    expires_at: "",
    candidate_type: "karyawan" as CandidateType,
  });
  const [bulkForm, setBulkForm] = useState({
    count: 300,
    prefix: "DOV",
    name_prefix: "Kandidat",
    position_applied: "",
    start_number: 1,
    expires_at: "",
    candidate_type: "karyawan" as CandidateType,
  ...
  async function onDelete(id: string, code: string) {
    if (!confirm(`Hapus kode ${code}? Data kandidat, dokumen, dan hasil tes tetap tersimpan.`))
      return;
    await del({ data: { id } });
  ...
  function onEditExpiry(id: string, current: string | null) {
    const cur = current
      ? new Date(new Date(current).getTime() - new Date().getTimezoneOffset() * 60000)
          .toISOString()
          .slice(0, 16)
      : "";
  ...
  async function onDeleteAll(e: React.FormEvent) {
    e.preventDefault();
    if (purgeText.trim().toUpperCase() !== "HAPUS SEMUA") {
      toast.error("Ketik persis: HAPUS SEMUA");
      return;
    }
    setPurging(true);
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Buat kode akses untuk kandidat login ke portal test.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <Button variant="outline" onClick={() => onBulkActive(true)}>
            <Power className="mr-2 h-4 w-4" /> Aktifkan Semua
          </Button>
          <Button variant="outline" onClick={() => onBulkActive(false)}>
            <PowerOff className="mr-2 h-4 w-4" /> Nonaktifkan Semua
          </Button>
          <Button
            variant="outline"
            onClick={() => {
          ...
          >
            <CalendarClock className="mr-2 h-4 w-4" /> Set Masa Berlaku Semua
          </Button>
          <Dialog
            open={purgeOpen}
            onOpenChange={(v) => {
          ...
          >
            <DialogTrigger asChild>
              <Button variant="destructive">
                <Trash2 className="mr-2 h-4 w-4" /> Hapus Semua Kode
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Hapus Semua Kode Kandidat</DialogTitle>
              </DialogHeader>
              <form onSubmit={onDeleteAll} className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  Seluruh kode akses ({codes.length} kode) akan dihapus dan kandidat tidak bisa
                  login lagi.
                  <b className="text-foreground">
                    {" "}
                    Data kandidat, berkas di Bank Dokumen, dan Bank Data Hasil tetap tersimpan
                  </b>{" "}
                  (kode lama disimpan sebagai arsip pada data kandidat).
                </p>
                ...
                <DialogFooter>
                  <Button type="submit" variant="destructive" disabled={purging}>
                    {purging ? "Menghapus..." : "Hapus Semua Kode"}
                  </Button>
                </DialogFooter>
              </form>
            </DialogContent>
          ...
          <Dialog open={open} onOpenChange={setOpen}>
            <DialogTrigger asChild>
              <Button>
                <Plus className="mr-2 h-4 w-4" /> Buat Kode
              </Button>
            </DialogTrigger>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Buat Kode Kandidat</DialogTitle>
              </DialogHeader>
              <div className="flex gap-2">
                <Button
                  ...
                  className="flex-1"
                >
                  <Plus className="mr-2 h-4 w-4" /> Satuan
                </Button>
                <Button
                  type="button"
                  size="sm"
                  ...
                  className="flex-1"
                >
                  <Layers className="mr-2 h-4 w-4" /> Massal
                </Button>
              </div>
              {createMode === "single" ? (
                <form onSubmit={onCreate} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <TypePicker
                      ...
                      onChange={(v) => setForm({ ...form, candidate_type: v })}
                    />
                  </div>
                  <div className="space-y-2">
                ...
                <form onSubmit={onBulk} className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={(e) =>
                          setBulkForm({ ...bulkForm, start_number: Number(e.target.value) })
                        }
                      />
                    </div>
                  </div>
                  <div className="space-y-2">
                    <Label>Tipe Kandidat *</Label>
                    <TypePicker
                      value={bulkForm.candidate_type}
                      onChange={(v) => setBulkForm({ ...bulkForm, candidate_type: v })}
                    />
                  </div>
                  <div className="space-y-2">
                    ...
                    <Input
                      ...
                      onChange={(e) =>
                        setBulkForm({ ...bulkForm, prefix: e.target.value.toUpperCase() })
                      }
                      placeholder="DOV"
                    />
                  </div>
                  <div className="space-y-2">
                    <Label>Prefix Nama Kandidat</Label>
                    <Input
                  ...
                  <div className="space-y-2">
                    <Label>Posisi Dilamar</Label>
                    <Input
                      value={bulkForm.position_applied}
                      onChange={(e) =>
                        setBulkForm({ ...bulkForm, position_applied: e.target.value })
                      }
                    />
                  </div>
                  <div className="space-y-2">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Semua kode dibuat dalam status <b>aktif</b> dan langsung bisa dipakai kandidat
                    login.
                  </p>
                  <DialogFooter>
                    <Button type="submit" disabled={bulkSaving}>
                      {bulkSaving ? "Membuat..." : `Buat ${bulkForm.count} Kode`}
                    </Button>
                  </DialogFooter>
                </form>
              )}
      ...
      <Dialog open={autoOpen} onOpenChange={setAutoOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Otomatis Buat Kode</DialogTitle>
          </DialogHeader>
          <form onSubmit={onAuto} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setAutoForm({ ...autoForm, count: Number(e.target.value) })}
              />
            </div>
            <div className="space-y-2">
              <Label>Tipe Kandidat *</Label>
              <TypePicker
                value={autoForm.candidate_type}
                onChange={(v) => setAutoForm({ ...autoForm, candidate_type: v })}
              />
            </div>
            <div className="space-y-2">
              <Label>Masa Berlaku (opsional)</Label>
              <Input
                ...
                onChange={(e) => setAutoForm({ ...autoForm, expires_at: e.target.value })}
              />
              <p className="text-[11px] text-muted-foreground">
            ...
            <p className="text-xs text-muted-foreground">
              Kode akan dibuat dengan prefix <b>DOV</b>, nama <b>Kandidat 001..</b>, dan langsung
              aktif.
            </p>
            <DialogFooter>
              <Button type="submit" disabled={autoSaving}>
                {autoSaving ? "Membuat..." : `Buat ${autoForm.count} Kode`}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={expiryOpen} onOpenChange={setExpiryOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Set Masa Berlaku Semua Kode</DialogTitle>
          </DialogHeader>
          <form onSubmit={submitBulkExpiry} className="space-y-4">
            <div className="space-y-2">
              ...
              />
              <p className="text-[11px] text-muted-foreground">
                Kosongkan untuk menghapus batas waktu semua kode.
              </p>
            </div>
            <DialogFooter>
      ...
      <Dialog open={!!editExpiry} onOpenChange={(v) => !v && setEditExpiry(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Ubah Masa Berlaku Kode</DialogTitle>
          </DialogHeader>
          <form onSubmit={submitEditExpiry} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setEditExpiry((s) => (s ? { ...s, value: e.target.value } : s))}
              />
              <p className="text-[11px] text-muted-foreground">Kosongkan untuk menghapus batas.</p>
            </div>
            <DialogFooter>
              <Button type="submit">Simpan</Button>
            </DialogFooter>
          ...
          <div className="space-y-3 md:hidden">
            {codes.map((c: any, i: number) => (
              <div key={c.id} className="rounded-lg border p-3">
                <div className="grid grid-cols-[minmax(0,1fr)_auto] items-start gap-3">
                  <div className="min-w-0">
                    ...
                    <Badge variant="outline" className="mt-1">
                      {candidateTypeShort(c.candidate_type)}
                    </Badge>
                  </div>
                  <div className="flex shrink-0 flex-col items-end gap-2">
                    <div className="flex items-center gap-2">
                      <span
                        className={`text-xs font-medium ${c.active ? "text-success" : "text-muted-foreground"}`}
                      >
                        {c.active ? "Aktif" : "Off"}
                      </span>
                      <Switch
                        ...
                        aria-label={`Aktifkan kode ${c.code}`}
                      />
                    </div>
                    <Button
                      size="icon"
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2 border-t pt-2 text-xs">
                  ...
                  {c.candidates?.data_completed ? (
                    <Badge className="bg-success">Data lengkap</Badge>
                  ) : c.candidates ? (
                    <Badge variant="secondary">Data belum lengkap</Badge>
                  ) : (
                    <Badge variant="outline">Belum login</Badge>
                  )}
                  <span className="text-muted-foreground">
                    Digunakan: {c.used_at ? new Date(c.used_at).toLocaleDateString("id-ID") : "-"}
                  </span>
                  <button
                    onClick={() => onEditExpiry(c.id, c.expires_at)}
                    className="hover:underline"
                  >
                    {c.expires_at ? (
                      <span
                        className={
                          new Date(c.expires_at).getTime() < Date.now() ? "text-destructive" : ""
                        }
                      >
                        Berlaku s/d{" "}
            ...
            {codes.length === 0 && (
              <p className="py-8 text-center text-sm text-muted-foreground">
                Belum ada kode. Klik "Buat Kode" untuk mulai.
              </p>
            )}
          </div>
              ...
              <TableBody>
                {codes.map((c: any, i: number) => (
                  <TableRow key={c.id}>
                    ...
                    <TableCell>
                      <Badge variant="outline">{candidateTypeShort(c.candidate_type)}</Badge>
                    </TableCell>
                    <TableCell>
                      <PresenceBadge lastSeenAt={c.last_seen_at} />
                    </TableCell>
                    ...
                    <TableCell>
                      {c.used_at ? new Date(c.used_at).toLocaleDateString("id-ID") : "-"}
                    </TableCell>
                    <TableCell>
                      <button
                        onClick={() => onEditExpiry(c.id, c.expires_at)}
                    ...
                    <TableCell>
                      <div className="flex flex-col items-center gap-1">
                        ...
                        />
                        <span
                          className={`text-[10px] font-medium ${c.active ? "text-success" : "text-muted-foreground"}`}
                        >
                          {c.active ? "Aktif" : "Off"}
                        </span>
                      </div>
