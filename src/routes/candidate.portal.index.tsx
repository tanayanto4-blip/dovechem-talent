  ...
  head: () => ({
    meta: [
      { title: "Beranda Portal Kandidat — Dover Chemical" },
      {
        name: "description",
        content:
          "Ringkasan tahapan seleksi kandidat PT Dover Chemical: kelengkapan data diri dan progres psikotest.",
      },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Halo, {data?.candidate?.full_name ?? session.candidate_name}. Ikuti tahapan berikut untuk
          menyelesaikan proses seleksi Anda.
        </p>
      </div>
        ...
        <CardContent>
          <div className="mb-2 flex items-baseline justify-between">
            ...
            <span className="text-sm text-muted-foreground">
              {steps.filter(Boolean).length} / {steps.length} tahap selesai
            </span>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <StepCard
          title="Data Diri"
          desc="Isi biodata lengkap Anda."
          icon={User}
          done={!!dataDone}
          to="/candidate/portal/data"
        />
        <StepCard
          title="Psikotest"
          desc={`${finishedTests}/${totalTests} test selesai`}
          icon={ClipboardList}
          done={testsDone}
          to="/candidate/portal/tests"
        />
      </div>
      ...
      <Card className="border-primary/20 bg-primary/5 shadow-none">
        ...
        <CardContent className="space-y-3 text-sm text-muted-foreground">
          <p>
            <strong className="text-primary">1. Lengkapi Data Diri terlebih dahulu.</strong> Klik
            tombol <em>Mulai</em> pada kartu Data Diri, isi seluruh informasi yang diminta, lalu
            simpan.
          </p>
          <p>
            <strong className="text-primary">2. Ikuti petunjuk Psikotest.</strong> Setelah data diri
            tersimpan, petunjuk untuk mengerjakan psikotest akan muncul. Kerjakan setiap bagian
            sesuai arahan yang diberikan.
          </p>
          <p>
            Pastikan koneksi internet stabil dan kerjakan tes secara mandiri. Jika mengalami
            kendala, hubungi tim HRD Dover Chemical.
          </p>
        </CardContent>
      </Card>
    </div>
    ...
    <Card className="shadow-card">
      <CardContent className="p-5">
        ...
        <Button asChild size="sm" variant={done ? "outline" : "default"} className="mt-4 w-full">
          <Link to={to}>
            {done ? "Lihat" : "Mulai"} <ArrowRight className="ml-1 h-3.5 w-3.5" />
          </Link>
        </Button>
      </CardContent>
    </Card>
