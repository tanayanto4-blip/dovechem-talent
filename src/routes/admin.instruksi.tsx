...
export const Route = createFileRoute("/admin/instruksi")({
  head: () => ({
    meta: [
      { title: "Instruksi Suara Test — Admin Dover Chemical" },
      {
        name: "description",
        content:
          "Susun dan unggah instruksi suara yang diputar kandidat sebelum memulai setiap psikotest.",
      },
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Tulis teks instruksi untuk tiap test. Teks ini akan dibacakan otomatis kepada kandidat
...
function VoiceCard({ row, onSave }: { row: Row; onSave: (p: any) => Promise<void> }) {
  ...
  const [audio, setAudio] = useState<{
    ...
    path: row.voice_audio_path,
    name: row.voice_audio_name,
    mime: row.voice_audio_mime,
    url: row.voice_audio_url,
  });
  const [uploading, setUploading] = useState(false);

  async function handleUpload(file: File) {
    if (!AUDIO_MIME.includes(file.type)) {
      toast.error("Format audio tidak didukung (gunakan MP3, WAV, M4A, OGG, atau WEBM).");
      return;
    }
    if (file.size > MAX_AUDIO_MB * 1024 * 1024) {
      toast.error(`Ukuran file maksimal ${MAX_AUDIO_MB} MB.`);
      return;
    }
    setUploading(true);
    try {
      ...
      toast.error(e?.message || "Gagal mengunggah audio.");
    } finally {
      setUploading(false);
    }
  }
  ...
  useEffect(() => {
    setText(row.voice_instruction ?? "");
  }, [row.voice_instruction]);
  ...
  async function save() {
    ...
    try {
      await onSave({
        ...
        voice_lang: lang,
        voice_rate: rate,
        voice_autoplay: autoplay,
        voice_mode: mode,
        voice_audio_path: audio.path,
        voice_audio_name: audio.name,
        voice_audio_mime: audio.mime,
      });
    } catch (e: any) {
    ...
    <Card className="shadow-card">
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {row.voice_instruction ? (
            <Badge className="bg-success">Terisi</Badge>
          ) : (
      ...
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <div className="flex flex-wrap items-center justify-between gap-2">
            ...
            <Button
              ...
              size="sm"
              variant="ghost"
              onClick={() => setText(TEMPLATE(row.name, row.test_type))}
            >
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="flex items-center justify-between rounded-md border p-3">
            <Label htmlFor={`e-${row.id}`} className="text-sm">
              Aktifkan suara
            </Label>
          ...
          <div className="space-y-1.5">
            <Label htmlFor={`l-${row.id}`} className="text-xs">
              Bahasa suara
            </Label>
            <Input
              id={`l-${row.id}`}
          ...
          <div className="space-y-1.5">
            <Label htmlFor={`r-${row.id}`} className="text-xs">
              Kecepatan ({rate.toFixed(2)}x)
            </Label>
            <Input
              id={`r-${row.id}`}
              type="range"
        ...
        <div className="space-y-3 rounded-lg border p-3">
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            <span className="text-xs text-muted-foreground">
              MP3, WAV, M4A, OGG, atau WEBM · maks {MAX_AUDIO_MB} MB
            </span>
            {audio.path && (
              <Button
                type="button"
                ...
                onClick={() => {
                  setAudio({ path: null, name: null, mime: null, url: null });
                  setMode("tts");
                }}
              >
          ...
          {audio.url ? (
            ...
            <p className="text-xs text-muted-foreground">
              Belum ada rekaman. Kandidat akan mendengar suara otomatis dari teks di atas.
            </p>
          )}
        </div>
