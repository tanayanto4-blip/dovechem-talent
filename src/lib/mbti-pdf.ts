...
const TYPE_NICK: Record<string, string> = {
  ...
  ISTP: "The Craftsman", ISFP: "The Composer", INFP: "The Healer", INTP: "The Thinker",
...
export function exportMbtiPdf(result: any, meta: MbtiPdfMeta) {
  ...
  const line = (text: string, size = 10, bold = false, color: [number, number, number] = [30, 30, 30]) => {
    ...
    if (y + wrapped.length * (size + 2) > pageH - margin) { doc.addPage(); y = margin; }
    doc.text(wrapped, margin, y);
    y += wrapped.length * (size + 2);
  };
  const gap = (h = 8) => { y += h; };
  const hr = () => { doc.setDrawColor(200); doc.line(margin, y, pageW - margin, y); y += 8; };
  ...
  hr();
  doc.setFont("helvetica", "bold"); doc.setFontSize(28); doc.setTextColor(15, 55, 110);
  ...
  pairs.forEach((pair) => {
    ...
    doc.text(`Dominan: ${dominant}   |   ${x}: ${cx}   ${yl}: ${cy}   |   Kejelasan: ${clarity}%`, margin, y + 14);
    // bar
    const barW = pageW - margin * 2;
    const total = Math.max(1, cx + cy);
    const leftW = (cx / total) * barW;
    doc.setFillColor(15, 55, 110); doc.rect(margin, y + 20, leftW, 8, "F");
    doc.setFillColor(200, 210, 225); doc.rect(margin + leftW, y + 20, barW - leftW, 8, "F");
    y += 40;
  });
  ...
  for (let i = 1; i <= total; i++) {
    ...
    doc.text(
      `PT Dover Chemical — Laporan MBTI — Halaman ${i}/${total}`,
      pageW / 2,
      pageH - 20,
      { align: "center" },
    );
