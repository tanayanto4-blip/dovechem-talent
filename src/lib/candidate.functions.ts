...
/* ------------------------------------------------------------------ */
/* Proctoring — candidate camera monitoring during a test              */
/* ------------------------------------------------------------------ */

const MAX_SNAPSHOT_BYTES = 1.5 * 1024 * 1024; // 1.5MB per frame

const SnapshotInput = z.object({
  code: z.string().trim().min(3).max(64),
  attempt_id: z.string().uuid(),
  event: z.enum(["snapshot", "camera_on", "camera_off", "camera_denied", "tab_hidden"]).default("snapshot"),
  base64: z.string().max(3 * 1024 * 1024).optional(),
});

/**
 * Store one webcam frame (or a camera event without an image) for the running
 * attempt. Images live in the private `candidate-files` bucket under
 * `proctor/<candidate_id>/...` and are only readable by Super Admin via a
 * short-lived signed URL.
 */
export const candidateProctorSnapshot = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const { data: attempt } = await sb
      .from("test_attempts")
      .select("id, candidate_id, test_id, status")
      .eq("id", data.attempt_id)
      .maybeSingle();
    if (!attempt || attempt.candidate_id !== cand.id) throw new Error("Attempt tidak valid.");

    let imagePath = "";
    if (data.base64) {
      const buf = Buffer.from(data.base64, "base64");
      if (buf.length === 0) throw new Error("Frame kosong.");
      if (buf.length > MAX_SNAPSHOT_BYTES) throw new Error("Ukuran frame terlalu besar.");
      imagePath = `proctor/${cand.id}/${attempt.id}/${Date.now()}.jpg`;
      const up = await sb.storage.from("candidate-files").upload(imagePath, buf, {
    ...
    const { error } = await sb.from("proctor_snapshots").insert({
      candidate_id: cand.id,
