...
export const listCandidateCodes = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const {
      ...
      .from("candidate_codes")
      .select("*, candidates(id, full_name, data_completed, updated_at)", { count: "exact" })
...
export const getAttemptDetail = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context, data }) => {
    ...
    const { data: attempt, error } = await supabaseAdmin
      .from("test_attempts")
      .select(
        "*, tests(*), candidates(id, full_name, code_snapshot, age, school_name, education, major, work_experience, job_position, job_level, phone, email, position_applied, gender, birth_date, birth_place, nik, address, marital_status, candidate_codes(code)), test_answers(*)",
      )
      .eq("id", data.id)
      .single();
    if (error) throw new Error(error.message);
